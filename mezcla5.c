...
typedef struct {
    ...
    const char *mensajeTarado;
    const char *mensajeSwitchCero;
    const char *mensajeSwitchContrario;
    const char *mensajeTimeoutSerial;
    const char *mensajeResultadosGuardados;
    const char *mensajeTodoApagado;
} Config;
...
Config cfg = {
    ...
    .serialBaud              = B9600,
    .serialSleep             = 1000,
    .mensajeTarado           = "Cuidado al tarar la balanza durante el proceso, esto se debe realizar antes de iniciar el programa.",
    .mensajeSwitchCero       = "Aviso: switch de flujo %d pasó a 0 durante la etapa.",
    .mensajeSwitchContrario  = "ERROR: switch de la bomba contraria %d activado durante la etapa.",
    .mensajeTimeoutSerial    = "Timeout leyendo balanza",
    .mensajeResultadosGuardados = "Resultados guardados en archivo: %s",
    .mensajeTodoApagado      = "** TODAS LAS SALIDAS APAGADAS **"
};
...
void guardarResultados(float kgAgua, float kgAFS40, float kgTotal) {
    ...
    fclose(archivo);
    printf(cfg.mensajeResultadosGuardados, nombreArchivo);
    printf("\n");
    fflush(stdout);
}
...
void llenarEtapa(
    ...
    *kgFluido = 0.0;
    ...
    if(*kgFluido < 0){
        apagarTodo();
        printf("\n%s\n", cfg.mensajeTarado);
        guardarError(cfg.mensajeTarado, *kgActual, *kgFluido);
        close(serialFd);
        exit(1);
    }
    ...
    while(*kgActual + 0.5 < objetivoEtapa){
        ...
        if(flujoActual != flujoPrevio){
            if(flujoActual == 0)
                printf(cfg.mensajeSwitchCero, flujoPin);
            flujoPrevio = flujoActual;
        }

        if(flujoContrario == 1){
            apagarTodo();
            printf("\n%s\n", cfg.mensajeSwitchContrario);
            close(serialFd);
            exit(1);
        }
    }
